...
// trims before emitting a URL, so those posts were advertised at a clean URL
// that then 404s, because the page lookup matches the slug exactly.
//
// Trimming the stored value fixes both ends. blog-inner.ts now trims in
// beforeChange, so this only has to clean up what is already there.
